...
$$
\mathcal{M}_c = \left\{x_c \in \mathbb{R}^{|G_{ligand}| \times 3} \;\middle|\; g(x_c) \approx 0\right\}
$$
...
$$
dZ^{(t)} = -\frac{1}{2}\bigl[p^{(t)}, 0\bigr] \, dt + \bigl[dB_{\mathbb{R}^{m \times 3}}, dB_{SO(3)^m}\bigr]
$$
...
$$
dZ_{\mathrm{rev}}^{(t)} = \left[\frac{1}{2}p^{(t)} + \nabla_p \log p_{T-t}\bigl(Z_{\mathrm{rev}}^{(t)} \mid G_{dock}\bigr),\; \nabla_R \log p_{T-t}\bigl(Z_{\mathrm{rev}}^{(t)} \mid G_{dock}\bigr)\right] \, dt
+ \left[dB_{\mathbb{R}^{m \times 3}}, dB_{SO(3)^m}\right]
$$
...
\left\| s_\theta\bigl(Z^{(t)}, t, G_{dock}\bigr) - \nabla_z \log p_{t \mid 0}\bigl(Z^{(t)} \mid Z^{(0)}\bigr) \right\|^2
\right]
$$
...
여기서 $U_{SO(3)^m}$는 $SO(3)^m$ 위의 uniform distribution을 뜻한다.
