...
### One-to-one token correspondence가 왜 중요한가

논문에서 은근히 중요한 설계 포인트가 하나 더 있다. 이 tokenizer는 input atom, latent position, token, reconstructed atom 사이에 **one-to-one positional correspondence**를 유지한다. 즉, “어떤 token이 어느 atom에 대응하는가”가 완전히 섞여버리는 set-compression 방식이 아니라, atom sequence를 따라 token sequence를 만든다.

이게 중요한 이유는 세 가지다.

1. **decoder가 훨씬 단순해진다**
   - token 하나가 대략 어떤 위치의 atom 정보를 복원해야 하는지가 정해져 있다
2. **reconstruction objective를 직접 걸기 쉽다**
   - per-atom RMSE나 intra-residue distance loss를 자연스럽게 계산할 수 있다
3. **downstream serialization과 연결이 쉽다**
   - discrete language model이나 sequence model이 token sequence를 그대로 다룰 수 있다

반대로 이 설계는 한계도 있다. atom ordering이나 serialization 방식의 영향을 받을 수 있고, point cloud를 truly permutation-invariant한 object로 다루지 않는다. 하지만 Bio2Token은 이 trade-off를 의식적으로 택한다. point cloud tokenizer에서 가장 중요한 것은 완전한 집합 대칭성보다, **길고 큰 all-atom 구조를 안정적으로 encode/decode하는 것**이라는 판단이다.

### Alignment and reconstruction geometry

논문은 reconstruction error를 계산할 때 decoded point cloud와 ground-truth point cloud를 먼저 **Umeyama-Kabsch alignment**로 맞춘다고 설명한다. 이 단계는 tokenizer 품질을 볼 때 꽤 중요하다.

왜냐하면 tokenizer가 rigid-body translation이나 global rotation 차이 때문에 불필요한 penalty를 받으면, 실제 local geometry fidelity를 제대로 측정하지 못하기 때문이다. alignment 이후 RMSE를 보는 것은 결국 다음 질문에 더 가깝다.

> **토큰이 전체 구조의 shape와 local geometry를 얼마나 잘 보존하는가?**

즉, 이 논문에서 reconstruction은 “절대 좌표를 그대로 복사했는가?”보다 **동일 구조를 다시 세울 수 있는가**에 더 가깝다. 이건 tokenizer를 downstream generative interface로 본다는 점과도 잘 맞는다.
...
### Dataset scale and what “generalization” means here

이 논문은 단순히 하나의 biomolecule family에서만 tokenizer를 맞춘 게 아니다. training/test setup도 꽤 넓다.

| Dataset | Role | Size / range |
|---|---|---|
| ∇²DFT | small molecule training/eval | 1.9M molecules, 16M conformers |
| CATH4.2 | protein training/eval | 약 18k structures |
| CASP14/15 | protein holdout eval | published-after-CATH holdout |
| RNA3DB | RNA training/eval | 최대 약 96k heavy atoms |
| AFDB subset | bio2token training | 100k structures |

이 표가 중요한 이유는, Bio2Token의 “범용성”이 단순 슬로건이 아니기 때문이다. unified tokenizer인 bio2token은 small molecules, proteins, RNA를 모두 본다. 그리고 complex는 학습에 직접 넣지 않았는데도 inference에서 어느 정도 복원한다. 즉, 이 논문이 말하는 generalization은 단순 test split generalization만이 아니라 다음을 포함한다.

- unseen conformers / unseen molecules / unseen scaffolds
- training보다 훨씬 큰 proteins와 RNA
- training에 없던 mixed complexes

이 점은 residue-level tokenizer와 비교할 때 특히 중요하다. residue-level tokenizer는 보통 단백질 한 종류의 문법 안에서만 잘 작동하는 경우가 많지만, Bio2Token은 아예 **atomistic point cloud 자체를 공통 언어로 삼는다**.
...
또 여기서 한 가지 더 보이는 건, unified tokenizer가 단순 평균적인 compromise가 아니라는 점이다. 보통 범용 모델은 각 domain-specific 모델보다 조금씩 밀릴 거라고 생각하기 쉽다. 그런데 Bio2Token은 proteins나 RNA 일부 setting에서는 오히려 domain-specific tokenizer보다 더 잘 나온다. 이건 atomistic point cloud 분포를 더 넓게 보는 것이 regularization처럼 작동했을 가능성을 시사한다.
...
개인적으로는 이 figure가 Bio2Token의 장점과 한계를 동시에 보여준다고 본다. 장점은, orientation-dependent token fluctuation이 있어도 reconstruction은 충분히 안정적이라는 점이다. 한계는, token 자체를 downstream discrete language model의 “의미 있는 단어”처럼 해석하려 하면 회전 의존성이 잡음을 만들 수 있다는 점이다. 그래서 downstream generative modeling에서는 rotation canonicalization, augmentation, 혹은 continuous latent와 discrete token의 hybrid 사용이 더 나을 수도 있다.
...
특히 이 논문은 representation learning의 우선순위를 다시 정렬한다. 보통 구조 모델을 설계할 때는 먼저 SE(3) symmetry를 얼마나 잘 넣을지를 고민한다. 그런데 Bio2Token은 오히려 “긴 sequence를 현실적으로 다룰 수 있느냐”를 더 앞세운다. 이 선택이 protein folding이나 docking처럼 물리 법칙이 더 직접적인 태스크에선 항상 옳다고 말할 수는 없다. 하지만 tokenizer처럼 **많은 구조를 싸게, 넓게, 일정 fidelity 이상으로 encode/decode**해야 하는 문제에서는 매우 현실적인 선택이다.
...
특히 tokenizer가 downstream에서 좋은 표현이 되려면 단순 reconstruction fidelity 외에도 다음이 중요하다.

- token distribution이 학습하기 쉬운가
- orientation/serialization noise에 얼마나 민감한가
- chemistry/biophysics와 얼마나 자연스럽게 연결되는가

Bio2Token은 첫 번째 단계, 즉 “all-atom tokenizer가 실용적인가?”에는 꽤 강한 답을 줬지만, “이 token이 downstream generation에서 가장 좋은 인터페이스인가?”는 아직 열린 질문으로 남는다.
