...
기존 패러다임과 비교하면 차이는 아래처럼 보인다.

| 축 | Diffusion / Flow | Drifting Models |
|---|---|---|
| 분포 진화가 일어나는 시점 | inference time | **training time** |
| inference 계산 | 다단계 네트워크 호출 | **single forward pass** |
| 학습 신호 | score / velocity / path supervision | **drifting field based fixed-point regression** |
| 핵심 구조 | ODE/SDE dynamics | **equilibrium-seeking drift dynamics** |
...
전체 과정을 개념적으로 그리면 다음과 같다.

```mermaid
graph TD
    A[Noise epsilon ~ p(epsilon)] --> B[Generator f_theta]
    B --> C[Generated sample x]
    C --> D[Feature extractor phi optional]
    D --> E[Compute drifting field V using real positives and generated negatives]
    E --> F[Build drifted target stopgrad(x + V)]
    F --> G[Regress generator output toward drifted target]
    G --> H[Update theta]
    H --> B
```
