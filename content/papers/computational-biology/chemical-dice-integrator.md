...
이 pattern은 특정 paper를 넘어 재사용 가치가 크다.

- expensive 3D/quantum teacher → cheap 2D/sequence student
- pocket-aware teacher → ligand-only prescreening student
- assay-rich teacher → structure-only deployable encoder
- graph + language + descriptor teacher → single molecular token interface

같은 구조로 확장할 수 있기 때문이다.

두 번째 저장 이유는 **representation quality와 representation availability를 동시에 평가**한다는 점이다. 좋은 embedding이더라도 5–10% molecule에서 source featurizer가 실패하면 실제 screening coverage가 깨진다. CDI-Generalised는 valid molecular string만 있으면 teacher latent의 proxy를 만들기 때문에 “성능”과 “coverage”를 같은 deployment contract 안에서 생각하게 한다.

세 번째는 evidence boundary가 비교적 명확하다. Nature Communications의 peer-reviewed article은 classification/regression benchmark, scaffold-based evaluation, low-data setting, featurizer failure coverage, 그리고 yeast damage-response assay까지 연결한다. 하지만 이 evidence는 **protein–ligand binding, target-disjoint generalization, therapeutic efficacy**를 직접 증명하지 않는다. 이 차이를 유지해서 읽을 가치가 있다.
...
| Article state inspected | peer-reviewed early Nature Communications publication; publisher notes further production edits may follow |
...
현재 official repository는 MIT license이고 architecture overview asset을 함께 배포한다. 결과 leaderboard screenshot보다 **teacher→student data flow**가 이 paper의 durable mechanism을 설명하는 데 더 중요하므로, 여기서는 한 개의 architecture visual만 사용한다.
...
*Source: the-ahuja-lab/ChemicalDice, `Images/CDI.png`, commit `b64ce09c…`, MIT license. 이 그림은 CDI의 multimodal integration concept를 설명하는 official project visualization이다. Paper benchmark를 독립적으로 검증하는 evidence가 아니다. Journal article은 six-view system을 canonical scientific claim으로 사용하며, current README 일부 문구가 five-view라고 남아 있는 문서 drift는 아래 reproducibility section에서 별도로 다룬다.*
...
각 view는 inductive bias가 다르다.

예를 들어 graph model은 adjacency와 local motifs를 자연스럽게 보지만 electronic observable을 직접 계산하지 않는다. Quantum descriptor는 전자구조 정보를 주지만 계산 비용이 크다. Bioactivity representation은 biological prior를 강하게 담을 수 있지만 upstream database coverage의 영향을 받는다. Molecular language model은 scalable하지만 input string representation에 의존한다.
...
Deployment에서 중요한 것은 오른쪽 아래 path다.
...
이것은 일반적인 multimodal fusion과 다르다. 보통 multimodal model은 train과 inference에서 모두 같은 modality set을 요구한다. CDI에서는 multimodal view가 **teacher target을 정의하는 training resource**이고, student의 deployment input은 single modality다.
...
그 다음 encoder $E_j$가 latent를 만들고 omitted view와 관련된 cross-modal commonality를 학습하도록 한다.
...
이 설계의 목적은 단순 concatenation이 아니다. `MOPAC 정보만 있는 차원`, `GROVER 정보만 있는 차원`을 그대로 쌓기보다 **다른 modalities로부터 한 modality를 얼마나 예측/정렬할 수 있는지**를 통해 shared information을 추출한다.

이것이 important distinction이다.
...
CDI Tier 1
    = learn what views can explain about one another
```

Official architecture docs는 이를 Leave-One-Out reconstruction/alignment objective로 설명한다.

### 구현 문서의 주의점

Current repository의 high-level architecture page와 low-level technical documentation 사이에는 reconstruction target notation이 완전히 동일하지 않은 부분이 있다. High-level description은 “other five views → omitted sixth view”를 강조하고, code-oriented technical documentation 일부 식은 autoencoder reconstruction term을 다른 방식으로 표기한다.

따라서 이 note에서는 **cross-modal leave-one-out commonality**라는 verified architectural contract까지만 강하게 해석하고, exact loss bookkeeping은 pinned implementation을 기준으로 재현해야 한다.

이런 문서 불일치는 reproducibility에서 사소하지 않다. Architecture name보다 실제 training code가 최종 contract다.

---

## 4. Tier 2 — six latent subspaces를 하나의 Super Embedding으로 압축한다
...
Official docs에서 중요한 점은 “8192”라는 숫자 자체보다 **representation boundary가 고정된다**는 것이다.

Upstream modalities는 서로 dimension과 preprocessing이 다르다. Current technical documentation은 대략적으로 MOPAC, GROVER, ChemBERTa, Signaturizer, ImageMol, Mordred가 수백에서 수만 차원의 서로 다른 source feature를 만든다고 설명한다. SEA 이후에는 모두 하나의 fixed-width vector contract로 바뀐다.
...
CDI-Basic은 rich하지만 deployment에는 불편하다.

새 molecule 하나를 넣을 때마다:

- MOPAC calculation
- Mordred descriptors
- graph encoder
- 2D image rendering + image encoder
- bioactivity signature model
- molecular language model

을 모두 돌리면 throughput이 낮고 failure surface도 넓다.

그래서 CDI-Generalised는 teacher embedding $z$를 target으로 삼아 raw molecular string에서 직접 예측한다.
...
Official public implementation docs는 student backbone을 Mamba State-Space Model로 설명한다.

Distillation의 최소 objective는 embedding regression이다.
...
Current technical documentation은 MSE 외 angular/cosine alignment도 사용한다고 설명한다. 따라서 conceptually는:
...
처럼 이해할 수 있다. 다만 exact weights/config는 paper prose보다 pinned code/checkpoint를 기준으로 확인해야 한다.

### 왜 Mamba인가

이 paper에서 Mamba 자체가 핵심 novelty는 아니다. Student는 long molecular strings를 scalable하게 처리하는 sequence encoder 역할을 한다.

핵심은:

$$
\text{architecture of student}
<
...
Transformer student로 바꿔도 “multimodal teacher → single-input deployable student”라는 핵심 실험 질문은 유지된다.

---

## 6. What is actually new: multimodality보다 train/deploy asymmetry가 더 중요하다

“여러 molecular features를 합친다”는 아이디어 자체는 새롭지 않다.

이 paper의 durable novelty를 세 층으로 나누는 편이 정확하다.
...
일부 source pipeline이 실패해도 valid SMILES에 대해 student embedding을 만들 수 있다.

이 중 가장 재사용 가치가 높은 것은 **B**다.

Model design 관점에서 중요한 질문은:

> “최종 deployment input이 모든 training-time information source를 실제로 다시 제공할 필요가 있는가?”

CDI의 답은 `no`다. Training-time privilege를 latent target으로 압축할 수 있다면 inference path를 훨씬 싸게 만들 수 있다.

---

## 7. Training data와 upstream priors를 representation 자체와 구분해야 한다

Official repository는 ChemicalDice가 ChEMBL 기반으로 학습되었다고 설명한다. 동시에 source modalities 자체도 독립적으로 pretrained knowledge를 가진다.

예를 들어 public implementation docs 기준:

- GROVER: large unlabeled molecular corpus에서 pretrained graph representation
- ChemBERTa: large SMILES corpus pretrained language model
- Signaturizer: broad bioactivity-profile prior
- ImageMol: pretrained image representation
...
=
F(
\text{multiple pretrained/expert representations of }m
).
$$

이것은 강점이면서 evaluation risk다.

Downstream benchmark와 upstream pretraining source 사이 overlap이 있을 수 있기 때문이다. 특히 bioactivity-derived representation은 task label과 의미적으로 가까운 prior를 가질 가능성이 있다.
...
이지만, 둘을 구분하려면 provenance audit가 필요하다.
...
- representation model이 frozen일 때도 test labels와 연결된 side information이 들어가는가?
...
Earlier public preprint는 23 classification datasets의 171 tasks와 10 regression datasets에 대한 broad benchmark를 보고했다. Peer-reviewed Nature Communications abstract는 CDI가 classical fusion methods보다 우수하고 established molecular descriptors와 같거나 나은 결과를 보였다고 요약한다.
...
Representation $z$가 달라져도 downstream model capacity가 충분히 크면 차이가 줄어들 수 있고, 반대로 작은 evaluator는 latent geometry의 linear separability를 더 강하게 평가한다.

그래서 “CDI embedding이 더 좋다”는 claim은:

- 어떤 downstream models를 썼는지
- hyperparameter selection budget이 동일했는지
- representation dimension 차이를 어떻게 처리했는지
- random/scaffold split 중 무엇인지
...
이는 random split보다 훨씬 meaningful하다. Close scaffold family를 train/test에 흩뿌리는 shortcut을 줄이기 때문이다.

Nature Communications abstract도 scaffold-based evaluation에서 stable generalization을 보고한다.

하지만 supported claim은 정확히:

> **new scaffold groups under the evaluated downstream protocol**

이다.

아래 claim은 별도 evidence가 필요하다.
...
Peer-reviewed article은 low-data evaluations에서 improved utility를 보고한다.

이 result의 의미는 단순히 “full-data average가 높다”보다 더 크다.

Frozen representation $z=f(m)$ 위에 작은 task head를 fit한다고 하자.

$$
\hat y=g_\phi(z).
$$

Label 수가 적을 때 downstream head가 새 chemistry를 처음부터 학습할 수 없다. 이때 teacher distillation으로 형성된 latent에 useful structure가 이미 있다면 sample complexity가 줄 수 있다.
...
다만 low-data advantage가 정말 broad transferable prior인지 보려면 다음을 확인해야 한다.

- label fraction만 줄였는가, scaffold diversity도 같이 줄었는가?
- hyperparameter tuning budget은 data fraction마다 동일한가?
- upstream representation corpus와 downstream test chemistry overlap이 있는가?
- class imbalance가 작은 $n$에서 더 심해지지는 않는가?

Low-data curve는 useful evidence지만 deployment claim은 split/provenance와 묶어서 봐야 한다.

---

## 11. Coverage is a first-class metric — 하지만 coverage와 fidelity는 다르다

Nature Communications abstract가 강조하는 결과 중 하나는 **individual feature-generation pipeline이 실패해도 complete embedding coverage를 유지한다**는 것이다.

이것은 deployment 관점에서 매우 practical하다.
...
C_T
=
P(\text{all required source featurizers succeed})
$$
...
C_S
=
P(\text{valid SMILES accepted by student})
$$

라고 하면 일반적으로 student는 훨씬 넓은 input domain을 가질 수 있다.

하지만 여기서 중요한 caveat가 있다.
...
이 네 항목을 분리해야 `100% embedding coverage`가 `100% trustworthy representation`으로 과대해석되지 않는다.

---

## 12. Ablation: modality 수가 줄면 무엇을 잃는가

Official repository는 six-view model에서 modality를 순차적으로 제거하는 ablation workflow를 공개한다.

Current docs에서는 MOPAC, Mordred, GROVER, Signaturizer 등을 차례로 제거하고 reconstruction-loss-based diagnostic을 기록한다.

이 ablation이 지지하는 가장 안전한 interpretation은:

> modalities가 완전히 redundant하지 않으며, full teacher space가 여러 complementary priors를 활용한다.
...
## 13. Encoder replacement experiment가 중요한 이유

Paper/repository narrative는 특정 encoder를 modality 자체와 동일시하지 않으려 한다.

예를 들어 `graph information`과 `GROVER`는 같은 것이 아니다.

$$
\text{modality}
\neq
\text{particular featurizer implementation}.
$$

이 distinction을 테스트하려면 같은 modality의 encoder를 교체해도 fused representation utility가 유지되는지 봐야 한다.

이 실험이 안정적이라면 teacher가 특정 model artifact를 암기하기보다 modality-level information을 활용한다는 해석이 더 강해진다.

이것은 future multimodal biomolecular pretraining에도 중요한 pattern이다.

- ligand graph encoder A ↔ B
- protein sequence encoder A ↔ B
- 3D conformer encoder A ↔ B
- assay descriptor A ↔ B

를 교체해도 student latent가 유지된다면 teacher interface가 implementation-specific하지 않다는 evidence가 된다.

---

## 14. Prospective evidence: yeast damage-response assay는 무엇을 증명하는가

Peer-reviewed article은 CDI representation으로 genome-stability-protective compounds를 prioritize한 뒤 **isoeugenol**과 **eugenyl acetate**를 yeast damage-response assay에서 experimental validation했다고 보고한다.

이것은 pure retrospective benchmark보다 강한 evidence다.
...
하지만 supported endpoint를 정확히 유지해야 한다.

이 experiment는:

- yeast system
- DNA-damage / genome-stability response
- selected compounds

에 대한 prospective biological signal을 지지한다.
...
## 15. 가장 중요한 confounder: teacher에 들어간 bioactivity prior

CDI의 강점 중 가장 흥미로운 modality는 Signaturizer이고, 동시에 가장 조심해서 읽어야 할 modality이기도 하다.

Bioactivity embedding은 molecular structure만으로부터 얻는 neutral geometric descriptor가 아니다. Upstream bioactivity databases와 learned association을 포함한다.

따라서 downstream task가 biological activity와 가까울수록:
...
이것은 “leakage가 있다”는 주장 아니다. 현재 public evidence만으로 leakage를 단정할 수 없다.

정확한 conclusion은:

> **bioactivity-informed representation의 OOD claim은 upstream provenance까지 포함해 audit해야 한다.**

이다.

---

## 16. Reproducibility: artifacts는 강하지만 documentation drift가 있다
...
하지만 current repository에는 작은 documentation drift가 있다.

Journal article과 architecture/technical docs는 **six modalities**를 canonical하게 설명한다. 반면 current top-level README의 일부 문장은 아직 **five modalities**라고 적는다.

이런 mismatch는 model science의 큰 오류는 아니지만 재현자가 어떤 configuration이 paper model인지 판단할 때 중요하다.

따라서 reproducible run은 최소한 다음을 pin해야 한다.
...
## 17. What is unsupported — 이 paper에서 넘어가면 안 되는 주장
...
## 18. 가장 decision-useful한 추가 실험

### Experiment A — teacher → student OOD retention ratio

같은 downstream evaluator를 고정하고:
...
- random
- scaffold-disjoint
- similarity-cluster-disjoint
- temporal
- assay/source-disjoint
- target/family-disjoint
...
그리고 quantum/3D teacher prior가 student OOD에 얼마나 남는지 측정한다.

이것이 CDI 아이디어를 3D molecular AI로 확장하는 가장 직접적인 실험이다.
...
Teacher featurizer가 실패하는 molecule만 별도 subset으로 모은다.
...
“coverage advantage”가 가장 필요한 subset에서 실제 quality가 유지되는지 확인할 수 있다.

---

## 19. Protein–ligand pretraining에 가져갈 때의 가장 중요한 변형

CDI의 pattern을 protein–ligand problem으로 옮길 때 teacher modality를 그대로 복사할 필요는 없다.

더 중요한 것은 **privileged-information teacher**라는 구조다.
...
이런 setup은 expensive pocket/3D/assay information을 pretraining target으로만 쓰고, deployment input이 제한된 상황에서도 그 prior를 얼마나 보존할 수 있는지 시험한다.

하지만 PL에서는 split contract를 더 강하게 해야 한다.
...
## 20. Complexity: 왜 teacher/student separation이 실제로 scalable한가
...
여기에는 MOPAC 같은 expensive computation과 여러 pretrained encoders가 포함된다.
...
따라서 screening library가 커질수록 distillation의 경제성이 커진다.

이 구조는 특히 teacher feature가 “한 번 계산하면 좋지만 매 molecule마다 반복하기 비싼” 경우에 적합하다.

- quantum calculations
- conformer ensemble generation
- docking-derived fingerprints
- large ensemble encoders

같은 feature가 좋은 후보다.

---

## 21. Falsification: 어떤 결과가 나오면 CDI story를 약하게 봐야 하나

좋은 representation story는 반증 조건이 있어야 한다.

다음 결과가 나오면 strong interpretation은 약해진다.
...
5. failure-subset에서 student coverage는 100%지만 error/uncertainty가 크게 악화된다.
6. 8192-D representation advantage가 dimension-matched random projection이나 larger downstream evaluator로 사라진다.

반대로 위 controls를 통과하면 **multimodal privileged teacher → single-input student**는 매우 강한 molecular representation primitive가 된다.
...
| Exact paper-vs-current-repo config drift | present; must pin revision |
...
1. **Training-time modality와 deployment-time modality는 같을 필요가 없다.**  
   Expensive quantum, bioactivity, graph, image priors를 teacher target에만 쓰고 cheap sequence student로 distill하는 것은 scalable molecular representation의 강한 설계 pattern이다.

2. **Coverage와 fidelity를 분리해서 평가해야 한다.**  
   Student가 valid SMILES 전체에 embedding을 제공하는 것은 operational advantage지만, source featurizer가 실패하는 chemistry에서 teacher information이 정확히 복원됐다는 뜻은 아니다.

3. **OOD claim은 downstream split뿐 아니라 upstream provenance까지 포함해야 한다.**  
   Scaffold split은 useful하지만 bioactivity/large-corpus pretrained teacher에서는 target, assay, temporal, upstream-overlap audit가 함께 있어야 representation transfer를 제대로 해석할 수 있다.
