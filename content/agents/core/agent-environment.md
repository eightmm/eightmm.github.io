...
| Type | Examples | Typical action | Verification |
| --- | --- | --- | --- |
| Read-only | search result, log, documentation, repository file | inspect and cite | source path, selected evidence |
| Editable | source file, Markdown note, configuration, generated artifact | patch or regenerate | diff, build, rendered check |
| External | API, CI system, deployment target, issue tracker | query or mutate | remote status, URL, audit log |
| Human | approval, feedback, task constraint, review decision | ask or wait | explicit response |
...
## Blog/Wiki Environment Map

| Environment | Truth source | Typical verifier |
| --- | --- | --- |
| Local content | `content/**/*.md` | diff, link check |
| Build output | `public/` generated by Quartz | `npx quartz build` |
| Git state | branch, staged files, commit | `git status`, `git log` |
| Public site | GitHub Pages deployment | Actions run status, rendered URL |
| Uncurated input | `content/inbox/` | publishing gate and sanitization check |
