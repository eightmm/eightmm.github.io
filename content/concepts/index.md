...
Concept note는 research note, paper summary, project page, infra note에서 반복해서 쓰는 재사용 가능한 용어를 정의합니다.

이 페이지는 전체 concept map입니다. 처음에는 hub에서 큰 축을 보고, 논문·프로젝트·포스트에서 정확한 정의가 필요할 때 개별 노트로 이동합니다.
...
- 논문 읽기: [[papers/workflows/paper-triage|Paper triage]] -> [[papers/workflows/paper-note-format|Paper note format]] -> [[papers/analysis/claim-extraction|Claim extraction]] -> [[papers/analysis/result-table-reading|Result table reading]] -> [[papers/analysis/benchmark-card|Benchmark card]] -> [[papers/analysis/ablation-map|Ablation map]] -> [[papers/reproducibility/reproduction-plan|Reproduction plan]] -> [[papers/workflows/reading-status|Reading status]]
- Coverage 확인: [[concepts/wiki-note-quality-gate|Wiki note quality gate]] -> [[concepts/coverage-matrix|Coverage matrix]] -> [[ai/paper-intake|AI paper intake]] -> [[molecular-modeling/paper-intake|Computational Biology paper intake]] -> [[math/formula-intake|Formula intake]] -> [[concepts/data/benchmark-intake|Benchmark intake]] -> [[posts/workflows/ai-molecular-math-post-intake|AI Computational Biology Math post intake]]
