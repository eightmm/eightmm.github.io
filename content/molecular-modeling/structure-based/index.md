...
Structure-based modeling은 AI 여부로 나누지 않습니다. Classical docking, force-field minimization, learned scoring, diffusion pose generation은 모두 같은 object와 evaluation problem을 공유합니다. 차이는 search space, scoring rule, learned component, evidence입니다.
...
| Family | Core operation | Evidence needed |
| --- | --- | --- |
| Docking/search | conformer and pose search, scoring | pose quality, enrichment, preparation protocol |
| Force-field refinement | energy minimization or dynamics | force field, time step, convergence, artifacts |
| Learned scoring | neural score or affinity predictor | split, baseline, calibration, leakage audit |
| Generative pose model | sample/refine coordinates | validity, diversity, pose plausibility, denominator |
| Structure-aware design | generate molecule/protein conditioned on structure | chemical validity, binding evidence, novelty, filters |
...
| Claim | Do not confuse with |
| --- | --- |
| pose prediction | binding affinity prediction |
| docking score | calibrated free energy |
| geometry validity | biological utility |
| high enrichment | good absolute affinity |
| generated molecule validity | synthesizability or target selectivity |
