...
- [[concepts/tasks/task-output-space|Task output space]]로 class, scalar, ranking, sequence, mask, graph, coordinate, action output을 구분하는 법
- [[concepts/tasks/retrieval|Retrieval]], [[concepts/tasks/similarity-search|similarity search]], [[concepts/tasks/reranking|reranking]]을 search pipeline의 서로 다른 단계로 구분하는 법
...
- [[molecular-modeling/index|Computational Biology]]를 구조기반/단백질/분자/유전체 입력으로 나눠 보는 이유
...
- [[molecular-modeling/structure-based/protein-ligand-docking|Protein-ligand docking]]에서 pose generation, scoring, filtering을 분리해야 하는 이유
...
- [[concepts/geometric-deep-learning/equivariance|Equivariance]]와 [[concepts/geometric-deep-learning/coordinate-frame|coordinate frame]]이 구조 기반 모델링에서 중요한 이유
...
- [[infra/gpu/index#memory|GPU memory]] 문제를 parameters, activations, optimizer state, KV cache로 나눠 보는 법
- [[infra/gpu/index#bottleneck-taxonomy|GPU bottleneck taxonomy]]로 utilization, memory, IO, communication 문제를 분리해서 진단하는 법
...
- [[concepts/systems/inference-capacity-planning|Inference capacity planning]]으로 context length, concurrency, KV cache, p95 latency를 연결하는 법
...
- [[agents/workflows/content-promotion-workflow|Content promotion workflow]]로 raw input을 inbox, concept, paper, project, infra, log, post로 승격하는 법
