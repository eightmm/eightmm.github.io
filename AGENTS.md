...
Keep `infra/` grouped by operational area rather than flat note names. Hardware foundations and speed hierarchy belong under `infra/hardware/`; GPU utilization, GPU memory, and bottleneck diagnosis belong under `infra/gpu/`; Slurm, scheduler-managed jobs, job arrays, checkpointing, and multi-node distributed training belong under `infra/hpc/`; serving and capacity planning belong under `infra/inference/`; training loop checks and single-node training behavior belong under `infra/training/`; data loading and storage issues belong under `infra/io/`; modules and containers belong under `infra/environments/`; run records belong under `infra/reproducibility/`. Add or update the nearest folder `index.md` when moving or creating an infra note.
