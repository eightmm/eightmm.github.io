...
- For coordinate-heavy notes, add or link a coordinate modeling contract. State coordinate source, frame, unit, transformation group, input features, output type, loss, metric, atom/residue mapping, and leakage risks.
...
Computational Biology scope should stay focused on sequence-based protein/genome modeling, molecular and ligand modeling, interaction modeling, structure-based modeling, docking, protein-ligand interaction, and data/evaluation boundaries. Do not open broad omics, transcriptomics, single-cell, pathway biology, clinical omics, or systems biology unless the user explicitly expands the scope.

In Computational Biology navigation, treat `entities/` and `molecular-modeling/entities.md` as the vocabulary layer. Protein, molecule, ligand, pocket, complex, assay, sequence, structure, and genome are objects; they should not all become peer top-level sidebar categories. Prefer route pages for visible navigation:

- Sequence-Based Modeling: protein sequence, genome sequence, variant windows, tokenization, sequence representation, family or region split.
- Molecular and Ligand Modeling: molecular identity, standardization, SMILES, graph, fingerprint, conformer, property prediction, molecular generation.
- Interaction Modeling: target-conditioned activity, affinity, selectivity, protein-ligand or protein-protein pairs, assay context.
- Structure-Based Modeling: pocket, pose, docking, scoring, virtual screening, structure-aware generation.
- Data and Evaluation: label semantics, split units, leakage, benchmark traps, calibration, uncertainty.
...
Infra and HPC notes should be general research-engineering knowledge, not a map of any private cluster. For new HPC notes, prefer generic concepts such as resource scheduling, resource requests, job arrays, distributed training, checkpointing, preemption/resume, GPU memory, storage IO, environment management, and reproducible run records. Hardware notes should cover public mental models such as CPU caches, SRAM, RAM, GPU VRAM, storage, network, latency, bandwidth, and bottleneck taxonomy without pretending to benchmark a private machine. Update `content/infra/index.md`, the relevant `content/infra/hpc/`, `content/infra/hardware/`, or `content/infra/server-ops/` index, and `content/concepts/systems/index.md` when the concept belongs to AI systems.

Keep `infra/` grouped by operational area rather than flat note names. Hardware foundations and speed hierarchy belong under `infra/hardware/`; GPU utilization, GPU memory, and bottleneck diagnosis belong under `infra/gpu/`; Slurm, scheduler-managed jobs, job arrays, checkpointing, and multi-node distributed training belong under `infra/hpc/`; data loading and storage issues belong under `infra/io/`; server operations belong under `infra/server-ops/`; public-safe run records, environment capture, and artifact boundaries belong under `infra/reproducibility/`. Put serving, capacity planning, training-run structure, environment management, and model lifecycle concepts under `concepts/systems/` unless the note is specifically about hardware, scheduler, storage, or operational runbooks. Add or update the nearest folder `index.md` when moving or creating an infra note.
...
For public paper reviews meant to read like a blog post, consult `content/papers/workflows/longform-paper-review-guide.md` before drafting. Target a 20-30 minute beginner-friendly review with a clear problem, background, method walkthrough, core formulas, evidence, limitations, concept map, and takeaways. Use tables and diagrams when they improve readability, and link AI, Math, Computational Biology, Infra, Agents, Concepts, and related Papers where they help the reader.

Keep `papers/` readable as a paper library, not a dumping ground for every paper-method helper. The visible Explorer shelves should stay broad: `papers/architectures/`, `papers/llm/`, `papers/generative-models/`, and `papers/computational-biology/`. Narrower support buckets such as `papers/sbdd/`, `papers/protein-modeling/`, `papers/learning-methods/`, and `papers/systems/` may keep stable URLs and domain-specific indexes, but should not make the sidebar noisy unless the user asks. Paper-reading process notes belong under `papers/workflows/`; claim/evidence/benchmark/ablation/limitation/comparison notes belong under `papers/analysis/`; artifact, checklist, implementation readiness, plan, and result notes belong under `papers/reproducibility/`. Update `content/papers/index.md` and the nearest folder index together.
...
- For foundational papers that should become public explanatory reviews, use `content/papers/workflows/longform-paper-review-guide.md` and keep the article readable in a 20-30 minute reading budget.
...
- For papers that should update the wiki graph, use `content/papers/workflows/paper-to-wiki-extraction.md` to decompose each claim into object, representation, method, formula, data, evidence, artifact, public boundary, and post-route updates before editing several support notes.
...
- Architecture-centric papers belong under `content/papers/architectures/` unless the stronger claim is LLM behavior, learning objective, generation, systems, or computational biology evaluation. LLM papers belong under `content/papers/llm/` when the central claim is language modeling, context use, retrieval, instruction tuning, alignment, tool use, or agent-facing behavior.
